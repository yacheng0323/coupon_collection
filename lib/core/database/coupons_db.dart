...
class CouponsDB {
  ...
  Future<InsertResult> insert({
    ...
    try {
      ...
      await database.rawInsert(
          '''INSERT INTO $tableName (store_name,type,start_time,end_time,content,image_path) VALUES (?,?,?,?,?,?)''',
          [store_name, type, start_time, end_time, content, imagepath]);
  ...
  Future<UpdateResult> update({
    ...
    try {
      ...
      if (type != null) updatedData["type"] = type;
      ...
      await database.rawUpdate(
          '''UPDATE $tableName SET store_name = ?, type = ?,start_time = ?,end_time = ?,content = ?,image_path = ? where id = ?''',
          [store_name, type, start_time, end_time, content, image_path, id]);
